...
**Personalized layout** — letters from an individual use the **official JHM personalized-letterhead layout** (brand.hopkinsmedicine.org → Design Standards → Letterhead; specs below). Reproduce it exactly:
...
- **Address block:** first line department/entity name in **bold**, then street address, City, State ZIP, phone ("T"), fax ("F"), and the author's @jhmi.edu email. **Canonical department addresses:** JHH → "Johns Hopkins Medicine, Department of Surgery" / 600 N. Wolfe Street / Baltimore, MD 21287; Bayview → "Johns Hopkins Medicine, Department of Surgery" / Johns Hopkins Bayview Medical Center / 4940 Eastern Avenue / Baltimore, MD 21224. Placeholders only for phone/fax/email.
...
- **General layout** (no personalization block): address block at **1" from left, 1/2" from top** — department name in bold as the first line, then the canonical address — brandmark 1-5/8" top-right. The official `Letterhead_JHM_JHH.dot` template is the general form. **Official print specs (verbatim):** Size: 8-1/2" × 11". Color: Brandmark prints JHM Blue (PMS 288) and JHM Gold (PMS 7406); the address block prints JHM Blue; 2/0 print job with no bleed. Brandmark size: 1-5/8". Typography: address information 8 pt font, 10 pt spacing/leading (8/10 pt) Gill Sans Regular; Entity Name / Department Name if needed is typeset in Gill Sans Bold. Paper: Strathmore Writing, Bright White, Wove finish, 24 lb. Text. Printing: Offset Lithography.
- **Rules from the standard:** letterhead is for people employed by or officially connected to Johns Hopkins, for official activities only; **no other institution's logos or co-branding**; second sheets are blank. **All header text is JHM Blue (`#002D72`), Gill Sans in print, Arial in Word** — never black, never another typeface.
...
- **Embed the actual logo** in every generated file, following the logo rules above: plain JHM mark on the deck title slide only (`assets/H-JHM-2.png`; `-2R` on dark backgrounds), full hospital lockup on posters (`assets/` ships the official `H/V-JHH-…png` and `H/V-Bayview-…png`), user's choice elsewhere. If the packaged assets are unavailable (e.g., the user attached or fetched the standalone markdown alone), **ask the user first, then download from the public repository**: tell them "I'll download the official [asset name] from the JH Surgery Format GitHub repository (github.com/davidstonko/jh-surgery-format) — OK?" and on approval fetch it from `https://raw.githubusercontent.com/davidstonko/jh-surgery-format/main/skill/assets/<filename>` (photos under `assets/photos/<filename>`) and embed it. Only if the download fails (restricted network in the build environment) fall back to a clearly marked placeholder box labeled with the exact filename to drop in — never screenshot or redraw a logo. The same fallback applies to the campus photos: a labeled `[campus photo — JHH or Bayview]` box. For print posters, remind the user to swap the PNG for the EPS from the brand portal before sending to the printer.
...
These three are the approved go-to images, shipped with the installed skill package (attach-only users of the standalone markdown: use the labeled `[campus photo — JHH or Bayview]` placeholder per Step 6 instead):
...
These are historical measurements of the official 4:3 .potx masters, kept for context. **Where anything here differs from the skill's design system (SKILL.md Step 3), SKILL.md wins** — including type scale and logo selection:
...
- **General letterhead:** address block at 1" from left, 1/2" from top, 5" wide; brandmark 1-5/8" top-right; entity name in bold as the first address line; second sheets blank; no affiliate or other-institution logos or co-branding ever. **Print specs (verbatim from the standard):** Size 8-1/2" × 11" · Color: brandmark prints JHM Blue (PMS 288) and JHM Gold (PMS 7406), address block prints JHM Blue, 2/0 print job with no bleed · Brandmark size 1-5/8" · Typography: address information 8 pt font, 10 pt spacing/leading (8/10 pt) Gill Sans Regular; Entity Name / Department Name if needed typeset in Gill Sans Bold · Paper: Strathmore Writing, Bright White, Wove finish, 24 lb. Text · Printing: Offset Lithography.
...
- **Canonical Department of Surgery addresses:** JHH — Johns Hopkins Medicine, Department of Surgery, 600 N. Wolfe Street, Baltimore, MD 21287 · Bayview — Johns Hopkins Bayview Medical Center, 4940 Eastern Avenue, Baltimore, MD 21224 ([source](https://www.hopkinsmedicine.org/surgery/locations/surgery-bayview-medical-center))
...
- **Generated decks should mirror the sanctioned templates' colors, typography, and motifs** (Heritage Blue + gold system, Arial) even when building 16:9 decks from scratch — but logo selection follows SKILL.md: plain JHM on deck title slides, hospital lockups on posters.
